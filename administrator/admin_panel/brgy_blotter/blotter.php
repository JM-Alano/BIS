...
$types = $conn->query("SELECT DISTINCT type FROM barangay_blotter WHERE type IS NOT NULL ORDER BY type");
$statuses = $conn->query("SELECT DISTINCT status FROM barangay_blotter WHERE status IS NOT NULL ORDER BY status");
...
if ($result->num_rows > 0) {?>
    ...
    <div class="limit-selector">
        ...
        <form method="GET" action="" class="filter-form">
                ...
                <select name="type_filter" id="type_filter">
                    ...
                    <?php while ($row = $types->fetch_assoc()): ?>
                        <option value="<?php echo $row['type']; ?>" 
                            <?php echo $type_filter == $row['type'] ? 'selected' : ''; ?>>
                            <?php echo $row['type']; ?>
                        </option>
                    <?php endwhile; ?>
                </select>
            ...
            <div class="filter-group">
                ...
                <select name="status_filter" id="status_filter">
                    ...
                    <option value="2" <?php echo $status_filter == '2' ? 'selected' : ''; ?>>Settled</option>
                    <option value="3" <?php echo $status_filter == '3' ? 'selected' : ''; ?>>Scheduled</option>
                </select>
            </div>
        ...
        <form method="GET" action="">
            ...
            <input type="hidden" id="complainant_filter" name="complainant_filter" value="<?php echo htmlspecialchars($complainant_filter); ?>">
            <input type="hidden" id="year_filter" name="year_filter" value="<?php echo htmlspecialchars($year_filter); ?>">
            <input type="hidden" id="type_filter" name="type_filter" value="<?php echo htmlspecialchars($type_filter); ?>">
            <input type="hidden" id="status_filter" name="status_filter" value="<?php echo htmlspecialchars($status_filter); ?>">
    ...
    <div class="pagination">
        <?php if ($page > 1): ?>
            <a href="?page=<?php echo $page - 1; ?>&limit=<?php echo $limit; ?>&complainant_filter=<?php echo urlencode($complainant_filter); ?>&year_filter=<?php echo urlencode($year_filter); ?>&type_filter=<?php echo urlencode($type_filter); ?>&status_filter=<?php echo urlencode($status_filter); ?>">Previous</a>
        <?php endif; ?>

        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="?page=<?php echo $i; ?>&limit=<?php echo $limit; ?>&complainant_filter=<?php echo urlencode($complainant_filter); ?>&year_filter=<?php echo urlencode($year_filter); ?>&type_filter=<?php echo urlencode($type_filter); ?>&status_filter=<?php echo urlencode($status_filter); ?>" 
        ...
        <?php if ($page < $totalPages): ?>
            <a href="?page=<?php echo $page + 1; ?>&limit=<?php echo $limit; ?>&complainant_filter=<?php echo urlencode($complainant_filter); ?>&year_filter=<?php echo urlencode($year_filter); ?>&type_filter=<?php echo urlencode($type_filter); ?>&status_filter=<?php echo urlencode($status_filter); ?>">Next</a>
        <?php endif; ?>
    </div>
...
document.getElementById('export-btn').addEventListener('click', function() {
    ...
    const complainantFilter = document.getElementById('complainant_filter').value;
    ...
    addHiddenInput('complainant_filter', complainantFilter);
    addHiddenInput('year_filter', yearFilter);
    addHiddenInput('type_filter', typeFilter);
